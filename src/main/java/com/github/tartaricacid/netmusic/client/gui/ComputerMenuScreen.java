...
public class ComputerMenuScreen extends AbstractContainerScreen<ComputerMenu> {
    ...
    protected void init() {
        ...
        this.readOnlyButton = new Checkbox(leftPos + 58, topPos + 55, 80, 20,
                new TranslatableComponent("gui.netmusic.cd_burner.read_only"), false);
    ...
    private void initUrlEditBox() {
        ...
        urlTextField = new EditBox(getMinecraft().font, leftPos + 10, topPos + 18, 120, 16, new TextComponent("Music URL Box"));
    ...
    private void initNameEditBox() {
        ...
        nameTextField = new EditBox(getMinecraft().font, leftPos + 10, topPos + 39, 120, 16, new TextComponent("Music Name Box"));
    ...
    private void initTimeEditBox() {
        ...
        timeTextField = new EditBox(getMinecraft().font, leftPos + 10, topPos + 61, 40, 16, new TextComponent("Music Time Box"));
    ...
    private void handleCraftButton() {
        ...
        if (StringUtils.isBlank(urlText)) {
            this.tips = new TranslatableComponent("gui.netmusic.computer.url.empty");
            return;
        }
        String nameText = nameTextField.getValue();
        if (StringUtils.isBlank(nameText)) {
            this.tips = new TranslatableComponent("gui.netmusic.computer.name.empty");
            return;
        }
        String timeText = timeTextField.getValue();
        if (StringUtils.isBlank(timeText)) {
            this.tips = new TranslatableComponent("gui.netmusic.computer.time.empty");
            return;
        }
        if (!TIME_REG.matcher(timeText).matches()) {
            this.tips = new TranslatableComponent("gui.netmusic.computer.time.not_number");
            return;
        }
        ...
        if (URL_FILE_REG.matcher(urlText).matches()) {
            ...
            if (!file.isFile()) {
                this.tips = new TranslatableComponent("gui.netmusic.computer.url.local_file_error");
                return;
            }
            try {
        ...
        this.tips = new TranslatableComponent("gui.netmusic.computer.url.error");
    }
    ...
    public void render(PoseStack poseStack, int x, int y, float partialTicks) {
        ...
        if (StringUtils.isBlank(urlTextField.getValue()) && !urlTextField.isFocused()) {
            font.draw(poseStack, new TranslatableComponent("gui.netmusic.computer.url.tips").withStyle(ChatFormatting.ITALIC), this.leftPos + 12, this.topPos + 18, ChatFormatting.GRAY.getColor());
        }
        if (StringUtils.isBlank(nameTextField.getValue()) && !nameTextField.isFocused()) {
            font.draw(poseStack, new TranslatableComponent("gui.netmusic.computer.name.tips").withStyle(ChatFormatting.ITALIC), this.leftPos + 12, this.topPos + 39, ChatFormatting.GRAY.getColor());
        }
        if (StringUtils.isBlank(timeTextField.getValue()) && !timeTextField.isFocused()) {
            font.draw(poseStack, new TranslatableComponent("gui.netmusic.computer.time.tips").withStyle(ChatFormatting.ITALIC), this.leftPos + 11, this.topPos + 61, ChatFormatting.GRAY.getColor());
        }
